...
#### 2. Permission-классы для CBV:

| № | Название права               | Описание                                                                                                              |
|---|------------------------------|-----------------------------------------------------------------------------------------------------------------------|
| 1 | `IsProfileOwnerOrAdminMixin` | Mixin для Django CBV: возвращает True, если пользователь является или "владельцем объекта" или "действующим админом". |
...
|---|----------------------------------|--------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|----------------------------|
| 1 | **RegisterPageView**             | `AnonymousOnlyMixin, FormView` | 1) Регистрации нового пользователя в системе (всегда первоначально регистрируется с профилем "Клиент"); <br/> 2) Отправка письма верификации. | `AppUser`, `ClientProfile` | `AppUserRegistrationForm`  |
| 2 | **VerifyEmailView**              | `AnonymousOnlyMixin, View`     | Верификация email после регистрации по `uid`+`token` (активация `is_active=True`)                                                            | `AppUser`                 | -                          |
| 3 | **LoginPageView**                | `AnonymousOnlyMixin, LoginView` | Вход ранее зарегистрированного пользователя в систему                                                                                        | `AppUser`                 | `AppUserLoginForm`         |
...
| 2 | `login_page.html`                 | Страница для авторизации пользователя в приложении                                                                                                                                    |
| 3 | `ratelimited.html`                | Страница для отображения понятной страницы для случаев превышения лимитов запросов (django-ratelimit). Красивое информирование о блокировке при борьбе с анти-спамом (ошибки 403/429) |
| 4 | `password_reset_request_page.html` | Страница для запроса восстановления пароля по email (неавторизованный пользователь)                                                                                                   |
| 5 | `password_reset_confirm_page.html` | Страница для подтверждения сброса пароля по uid/token и установке нового пароля                                                                                                       |
| 6 | `change_password.html`            | Страница для изменения пароля                                                                                                                                                         |
| 7 | `edit_client.html`                | Страница для редактирвоания данных пользователя                                                                                                                                       |
...
| 2 | `verify-email/`          | `GET`         | Активация пользователя / Подтверждение email (uid+token)                                                                      |
| 3 | `login/`                 | `POST`        | Авторизация (Login) ранее зарегистрированного пользователя в системе                                                          |
| 4 | `logout/`                | `POST`        | Выход (Logout)                                                                                                                |
| 5 | `password-reset/`        | `POST`        | Восстановление пароля по email (неавторизованный пользователь)                                                                |
| 6 | `password-reset-confirm/` | `GET`, `POST` | Подтверждение сброса пароля по uid/token и установке нового пароля                                                            |
| 7 | `password-change/`       | `POST`        | Изменение пароля (авторизованный пользователь)                                                                                |
| 8 | `profile-edit/`          | `GET`, `POST` | Редактирования пользовательских данных                                                                                        |
