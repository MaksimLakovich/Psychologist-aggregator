...
3. Для **API-эндпоинтов** созданы сериализаторы для ***авторизации (login)*** пользователя (получение JWT-токенов, позволяющий вход по email) и для ***выхода (logout)*** пользователя (помещение токена в blacklist refresh token):
...
| ----------------------------- |-------------------------|-------------------------------------------------------------------------------------|---------------------------------------------------------------------------------|--------------------------------------------------------------------------|
| **ChangePasswordView**        | `UpdateAPIView`         | Позволяет аутентифицированному пользователю сменить пароль, указав старый и новый.  | `AppUser`, `ChangePasswordSerializer`                                           | Правильно через отдельный эндпоинт.                                      |
...
| 3 | ⚠️ `/api/users/verify-email/`       | `GET`       | Подтверждение email после регистрации. ⚠️Отдельной "ручки" нет - этоф фоновый маршрут для работы клика по ссылке в email.    |
...
| `/api/users/change-password/`      |                         | Отдельный эндпоинт для смены пароля (с валидацией старого).                 |                                                   |                                                                                |                                                            |
