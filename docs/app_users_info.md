...
#### Для **API-эндпоинтов** созданы следующие базовые сериализаторы:
...
| 18 | **PsychologistProfileRetrieveUpdateView** | `RetrieveUpdateAPIView`          | РАБОТА С ПРОФИЛЕМ (психолог): 1) Получить данные профиля психолога (read-сериализатор `PsychologistProfileReadSerializer`); 2) Обновить данные профиля психолога (write-сериализатор `PsychologistProfileWriteSerializer`); 3) Разрешать изменение many2many через списки id | `PsychologistProfile`                            | `PsychologistProfileReadSerializer`, `PsychologistProfileWriteSerializer` |
| 19 | **ClientProfileRetrieveUpdateView**      | `RetrieveUpdateAPIView`          | РАБОТА С ПРОФИЛЕМ (клиент): 1) Получить данные профиля клиента (read-сериализатор `ClientProfileReadSerializer`); 2) Обновить данные профиля психолога (write-сериализатор `ClientProfileWriteSerializer`); 3) Разрешать изменение many2many через списки id                | `ClientProfile`                                  | `ClientProfileReadSerializer`, `ClientProfileWriteSerializer`            |
|    |                                          |                                  |                                                                                                                                                                                                                                                                             |                                                  |                                                                          |


| `PsychologistProfilePublicDetailView`   | `RetrieveAPIView`                 | Публичная карточка психолога (детали, без чувствительных полей).                                                                                                                     | `PsychologistProfile`                             | `PsychologistProfileSerializer` (public)                                  |               ❗ планируется |
