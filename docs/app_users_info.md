...
1. Для **API-эндпоинтов** созданы следующие базовые сериализаторы:  
   ...
   - `AppUserSerializer`
   - `PsychologistProfileSerializer` - Read-сериализатор (используется для GET - создавать/редактировать вложенные объекты через него нельзя)
...
| 18 | **PsychologistProfileRetrieveUpdateView** | `RetrieveUpdateAPIView`          | РАБОТА С ПРОФИЛЕМ: 1) Получить данные профиля психолога (read-сериализатор `PsychologistProfileSerializer`); 2) Обновить данные профиля психолога (write-сериализатор `PsychologistProfileWriteSerializer`); 3) Разрешать изменение many2many через списки id | `PsychologistProfile`                            | `PsychologistProfileSerializer`  |
|    |                                           |                                  |                                                                                                                                                                                                                                                               |                                                  |                                  |
|    |                                           |                                  |                                                                                                                                                                                                                                                               |                                                  |                                  |
...
| `/api/users/psychologists/`              |                   `GET` | Публичный список психологов (фильтрация/пагинация).                   |   ❗ планируется |
| `/api/users/psychologists/<uuid:uuid>/`  |                   `GET` | Публичная карточка психолога (detail).                                |   ❗ планируется |
| `/api/users/clients/me/`                 |   `GET`, `PUT`, `PATCH` | Просмотр/редактирование своего `ClientProfile` (auth, role=client).   |   ❗ планируется |
