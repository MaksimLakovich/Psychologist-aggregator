...
| 17 | `AppUserRetrieveUpdateView`             | `RetrieveUpdateAPIView`           | Получить / обновить данные текущего пользователя (first_name, last_name, phone, timezone и т.д.). *Пароль блокируется*; админ может смотреть/обновлять всех (в зависимости от прав). | `AppUser`                                         | `AppUserSerializer`                                                       |               ❗ планируется |
| 18 | `PsychologistProfileRetrieveUpdateView` | `RetrieveUpdateAPIView`           | Просмотр/редактирование профиля психолога (автор - психолог; админ - может редактировать/верифицировать). Разрешать изменение many2many через списки id.                             | `PsychologistProfile`                             | `PsychologistProfileSerializer` (+ отдельный write-serializer если нужно) |               ❗ планируется |
| 19 | `PsychologistProfileListView`           | `ListAPIView`                     | Публичный список психологов (с возможностью фильтрации/пейджинга) - для поиска/каталога.                                                                                             | `PsychologistProfile`                             | `PsychologistProfileSerializer` (с минимальным набором)                   |               ❗ планируется |
| 20 | `PsychologistProfilePublicDetailView`   | `RetrieveAPIView`                 | Публичная карточка психолога (детали, без чувствительных полей).                                                                                                                     | `PsychologistProfile`                             | `PsychologistProfileSerializer` (public)                                  |               ❗ планируется |
| 21 | `ClientProfileRetrieveUpdateView`       | `RetrieveUpdateAPIView`           | Просмотр/редактирование профиля клиента.                                                                                                                                             | `ClientProfile`                                   | `ClientProfileSerializer`                                                 |               ❗ планируется |
| 22 | `AdminPsychologistModerationView`       | `APIView` / `Generic`             | Эндпоинт для админа: верификация профиля (`is_verified`, `is_all_education_verified`, коммент/отказ).                                                                                | `PsychologistProfile`, `Education`                | кастомный сериализатор                                                    | ❗ планируется (опционально) |
...
| 19 | `/api/users/me/`                                       |           `GET`, `PUT`, `PATCH` | Получить / обновить текущего пользователя (`AppUser`). (auth required)          |               ❗ планируется |
| 20 | `/api/users/psychologists/`                            |                           `GET` | Публичный список психологов (фильтрация/пагинация).                             |               ❗ планируется |
| 21 | `/api/users/psychologists/<uuid:uuid>/`                |                           `GET` | Публичная карточка психолога (detail).                                          |               ❗ планируется |
| 22 | `/api/users/psychologists/me/`                         |           `GET`, `PUT`, `PATCH` | Просмотр/редактирование своего `PsychologistProfile` (auth, role=psychologist). |               ❗ планируется |
| 23 | `/api/users/clients/me/`                               |           `GET`, `PUT`, `PATCH` | Просмотр/редактирование своего `ClientProfile` (auth, role=client).             |               ❗ планируется |
| 24 | `/api/users/admin/psychologists/<uuid:uuid>/moderate/` |                          `POST` | (Admin) Верификация профиля психолога (is_verified, комменты).                  | ❗ планируется (опционально) |
