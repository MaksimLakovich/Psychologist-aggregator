...
|---|---------------------|------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|--------------------------|
| 1 | **RegisterPageView** | `FormView`             | 1) Регистрации нового пользователя в системе (всегда первоначально регистрируется с профилем "Клиент"); <br/> 2) Отправка письма верификации. | `AppUser`, `ClientProfile` | `AppUserRegistrationForm` |
| 2 | **VerifyEmailView** | `View`                 | Верификация email после регистрации по `uid`+`token` (активация `is_active=True`)                                                            | `AppUser`                 | -                        |
| 3 |                     |                        |                                                                                                                                              |                           |                          |
| 4 | **LoginPageView**   | `LoginView`            | Вход ранее зарегистрированного пользователя в систему                                                                                        | `AppUser`                 | `AppUserLoginForm`       |
|   |                     |                        |                                                                                                                                              |                           |                          |
