...
| **CustomTokenObtainPairView** | `APIView` (наследуется от `TokenObtainPairView`) | Авторизация пользователя по email и паролю. Возвращает JWT-токен.                                                       | `AppUser`, `CustomTokenObtainPairSerializer`                                    | Кастомный вход по email вместо username.                                                    |
| **CustomTokenRefreshView**    | `APIView` (наследуется от `TokenRefreshView`)    | Обновление JWT access-токена по refresh-токену.                                                                         | -                                                                               | Стандартный эндпоинт SimpleJWT.                                                             |
