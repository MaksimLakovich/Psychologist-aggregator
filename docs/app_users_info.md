...
- Используется `throttle (anti-spam)` при отправки email для подтверждения регистрации.
...
| 3  | ⚠️ `/api/users/verify-email/`           | `GET`  | Подтверждение email после регистрации. ⚠️Отдельной "ручки" нет - это фоновый маршрут для работы клика по ссылке в email.     |
...
| 10 | ⚠️ `/api/users/password-reset-confirm/` | `POST` | Подтверждение в email после сброса пароля. ⚠️Отдельной "ручки" нет - это фоновый маршрут для работы клика по ссылке в email. |
|    |                                         |        |                                                                                                                              |
|    |                                         |        |                                                                                                                              |
|    |                                         |        |                                                                                                                              |
