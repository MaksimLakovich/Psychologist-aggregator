...
|---| ----------------------------- | ------------------------------------------------ |--------------------------------------------------------------------------------------------------------------------------|--------------------|-----------------------------------|
| 1 | **CustomTokenObtainPairView** | `APIView` (наследуется от `TokenObtainPairView`) | Авторизация пользователя по email и паролю. Возвращает JWT-токен.                                                        | `AppUser`          | `CustomTokenObtainPairSerializer` |



| Название контроллера          | Тип (ViewSet / Generic)                          | Описание функционала (docstring)                                                                                        | Используемые модели / сериализаторы                                             | Примечания                                                                                  |
| ----------------------------- | ------------------------------------------------ |-------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------|
| **RegisterPsychologistView**  | `GenericAPIView`                                 | Регистрация нового пользователя с ролью *психолога*. Создает `AppUser` и автоматически связанный `PsychologistProfile`. | `AppUser`, `PsychologistProfile`, `RegisterSerializer`                          | При нажатии на кнопку "Зарегистрироваться как психолог".                                    |
| **RegisterClientView**        | `GenericAPIView`                                 | Регистрация нового пользователя с ролью *клиента*. Создает `AppUser` и связанный `ClientProfile`.                       | `AppUser`, `ClientProfile`, `RegisterSerializer`                                | При нажатии на кнопку "Зарегистрироваться как клиент".                                      |
| **EmailVerificationView**     | `APIView`                                        | Подтверждение email после регистрации (через токен, ссылка на email).                                                   | `AppUser`                                                                       | Можно реализовать через отдельную модель валидации токена или JWT с claim `"verify_email"`. |
...
|---|------------------------------------|-------------------------|-------------------------------------------------------------------------------------------------------------------------------|
| 1 | `/api/users/login/`                | `POST`                  | JWT-аутентификация по email и паролю. Возвращает `access` и `refresh` токены.                                                 |
| 2 | `/api/users/token/refresh/`        | `POST`                  | Обновление `access`-токена по `refresh`-токену. Стандартный эндпоинт SimpleJWT. Не требует отдельного контроллера во views.py |



| №  | Эндпоинт                           | HTTP-методы             | Описание функционала                                                                                                                | Используемые модели                               | Используемые сериализаторы                                                     | Примечания                                                 |
|----|------------------------------------|-------------------------|-------------------------------------------------------------------------------------------------------------------------------------| ------------------------------------------------- |--------------------------------------------------------------------------------|------------------------------------------------------------|
| 1  | `/api/users/register/psychologist/` | `POST`                  | Регистрация нового психолога. Создает пользователя (`AppUser`) с ролью *psychologist* и автоматически профиль `PsychologistProfile`. | `AppUser`, `PsychologistProfile`                  | `RegisterSerializer`, `PsychologistProfileSerializer`                          | Роль передается во `view` через `context`, не из запроса.  |
| 2  | `/api/users/register/client/`      | `POST`                  | Регистрация нового клиента. Создает пользователя (`AppUser`) с ролью *client* и автоматически профиль `ClientProfile`.              | `AppUser`, `ClientProfile`                        | `RegisterSerializer`, `ClientProfileSerializer`                                | Аналогично, роль задается во `view`.                       |
...
| 8  | `/api/users/educations/`           | `GET`, `POST`           | Просмотр и добавление записей об образовании (для психолога).                                                                       | `Education`                                       | `EducationSerializer`                                                          | `POST` - только для авторизованного психолога.             |
| 9  | `/api/users/educations/<id>/`      | `GET`, `PATCH`, `DELETE` | Просмотр, редактирование или удаление конкретной записи об образовании.                                                             | `Education`                                       | `EducationSerializer`                                                          | Ограничено владельцем (`IsOwnerOrReadOnly`).               |
| 10 | `/api/users/change-password/`      |                         | Отдельный эндпоинт для смены пароля (с валидацией старого).                                                                         |                                                   |                                                                                |                                                            |
| 11 | `/api/users/logout/`               |                         | Просто blacklisting refresh-токена, если включено в настройках SimpleJWT.                                                           |                                                   |                                                                                |                                                            |
