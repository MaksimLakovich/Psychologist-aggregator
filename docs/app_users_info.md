...
- `role_required_mixin.py` - миксины, для управления доступами к CBV на основе роли пользователя:
    
    | Название константы                            | Значение                                                                          |
    |-----------------------------------------------|-----------------------------------------------------------------------------------|
    | `RoleRequiredMixin(AccessMixin)`              | Базовый mixin для доступов на основе role в обычных Django CBV                    |
    | `ClientRequiredMixin(RoleRequiredMixin)`      | Разрешает доступ только **клиенту**, при этом может опционально пускать **гостя** |
    | `PsychologistRequiredMixin(RoleRequiredMixin)` | Разрешает доступ только **психологу**                                             |
...
#### edit_psychologist_form.py:
- Редактирование данных специалиста:
  - `EditPsychologistAccountForm(forms.ModelForm)` - форма для редактирования данных ***аккаунта*** специалиста.
  - `EditPsychologistProfileForm(forms.ModelForm)` - форма для редактирования данных ***профиля*** специалиста.
  - `PsychologistEducationForm(forms.ModelForm)` - форма для редактирования данных об ***образовании*** специалиста.
...
| № | Название контроллера          | Тип (ViewSet / Generic)         | Описание функционала (docstring)                  |
|---|-------------------------------|---------------------------------|---------------------------------------------------|
| 1 | **EditClientProfilePageView** | `ClientRequiredMixin, FormView` | Web-контроллер для редактирования профиля клиента |

#### edit_psychologist_view.py

| № | Название контроллера                | Тип (ViewSet / Generic)                   | Описание функционала (docstring)                      |
|---|-------------------------------------|-------------------------------------------|-------------------------------------------------------|
| 1 | **EditPsychologistProfilePageView** | `PsychologistRequiredMixin, TemplateView` | Web-контроллер для редактирования профиля специалиста |
...
| 8  | `edit_client.html`                             | Страница для редактирвоания данных клиента                                                                                                                                                     |
| 9  | `edit_psychologist.html`                       | Страница для редактирвоания данных специалиста                                                                                                                                                 |
| 10 | `_modal_add_methods.html`                      | Модалка для добавления методов в профиле клиента, которые интересуют клиента                                                                                                                   |
| 11 | `_modal_add_topics.html`                       | Модалка для добавления тематик в профиле клиента, которые интересуют клиента                                                                                                                   |
| 12 | `_modal_add_psychologist_specialisations.html` | Модалка для добавления в профиле специалиста его специализации для карточки в каталоге                                                                                                         |
| 10 | `_modal_add_psychologist_methods.html`         | Модалка для добавления методов в профиле специалиста с которыми работает специалист                                                                                                            |
| 11 | `_modal_add_psychologist_topics.html`          | Модалка для добавления тематик в профиле специалиста с которыми работает специалист                                                                                                            |
...
| 9  | `profile-edit/`          | `GET`, `POST` | Редактирования данных клиента                                                                                                                                                                         |
| 10 | `ps-profile-edit/`       | `GET`, `POST` | Редактирования данных специалиста                                                                                                                                                                     |
