...
|----|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1  | `IsOwner`       | Возвращает True, если пользователь является владельцем объекта. Используется в контроллерах для ограничения доступа к операциям с чужими объектами.     |
| 2  | `IsActiveAdmin` | Возвращает True, если пользователь активен и является админом. Используется во views для предоставления доступа админам к операциям CRUD над объектами. |
 ...
 **EducationListCreateView** | `ListCreateAPIView`            | 1) Вывести список всех Education, принадлежащих текущему психологу. <br> 2) Создать новую запись Education.   | `Education` | `EducationSerializer` |
 **EducationDetailView**     | `RetrieveUpdateDestroyAPIView` | Получить / обновить / удалить конкретную запись Education, только если она принадлежит текущему пользователю. | `Education` | `EducationSerializer` |
...
| **EducationViewSet**          | `ModelViewSet`          | CRUD-операции с образованием (Education). Только владелец может создавать и изменять. | `Education`, `EducationSerializer`                                              | Использует `CreatorMixin` для автоматического указания создателя.        |
| **MethodViewSet**             | `ReadOnlyModelViewSet`  | Только чтение справочника методов терапии.                                          | `Method`, `MethodSerializer`                                                    | Наполняется через Django Admin.                                          |
...
| `/api/users/educations/`           | GET, POST               | Список Education + создание новой записи |
| `/api/users/educations/<uuid:pk>/` | GET, PUT, PATCH, DELETE | Детальный CRUD для Education             |
...
| `/api/users/educations/`           | `GET`, `POST`           | Просмотр и добавление записей об образовании (для психолога).               | `Education`                                       | `EducationSerializer`                                                          | `POST` - только для авторизованного психолога.             |
| `/api/users/educations/<id>/`      | `GET`, `PATCH`, `DELETE` | Просмотр, редактирование или удаление конкретной записи об образовании.     | `Education`                                       | `EducationSerializer`                                                          | Ограничено владельцем (`IsOwnerOrReadOnly`).               |
