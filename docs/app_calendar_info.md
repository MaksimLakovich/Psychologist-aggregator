# 📆 Календарь (бронирование и проведение встреч)

---

**Концепция реализации:** динамическая генерация и явное кодирование "Availability Rules + Events", без массовой генерации слотов в БД.

То есть:
- ❌ НЕ храним все слоты
- ✅ Храним правила доступности (рабочие часы, выходные)
- ✅ Храним только реальные события / брони
- ✅ Слоты считаем на лету
- ✅ Используем асинхронность + учитываем таймзоны клиентов и психолога
- ✅ Учитываем конкуренцию за слот (race condition). Решение проблемы: "два клиента одновременно пытаются забронировать один слот"
- ✅ Сразу проектируем под Google / Apple / Outlook

Это правильный вариант для SaaS-платформы с 1k+ специалистов.

---

**Ключевая архитектурная идея календаря**:
...
| Отправка уведомлений           | `django-notifications-hq`               | Уведомления клиентам и психологам                                           |
| Асинхронные задачи (напоминания) | `Celery` + `Redis`                      | Отправка email/SMS перед встречей                                           |
| Временные зоны                 | `pytz`, `django-timezone-field`         | Обязательны, если пользователи из разных регионов                           |
| Формы и фильтрация анкет       | `django-filter`                         | Фильтры по опыту, полу, цене и т.д.                                         |

---

**ФТ:**
- сервис создания рабочего графика психолога (рабочие дни, рабочие часы, время между сессиями, нерабочие дни/часы);
- генерация слотов в календаре психолога (автогенерация на 2 недели вперед);
- жизненный цикл слота (создание брони, подтверждение брони, запланирован, начато, завершено, отмена по причине клиента, отмена по причине психолога, перенос по причине клиента/психолога); 
- оценка встречи/отзыв после завершения (письмо с оценочной формой или всплывающее окно в вэб);
- интеграция с Google;
- отображение UI календаря с иконками кол-ва встреч на тек день + возможность открыть день и посмотреть слоты(записи) на день + отдельно историю (проведенные/отмененные);
- функция создания встречи самим психологом (должен иметь возможность видеть своих клиентов и подтянуть его ID);
- функция создания психологом периодических встреч со своим клиентом.
...
**django-ical**
- Позволяет экспортировать и импортировать календари в формате .ics. 
- Можно использовать для синхронизации с Google Calendar / Apple Calendar. 
- Не управляет слотами, а только синхронизирует.
Вывод: использовать как дополнение, если нужно интегрировать с внешними календарями.


Google официально предоставляет:
- Google Calendar API
- OAuth 2.0
- Webhooks (push notifications)  - это для изменения в Google Calendar и чтоб сразу в приложении нашем появились новые данные (опционально)
- SDK и REST API
...
- Авторизация (**auth_form.py**):
  - `AppUserLoginForm(AuthenticationForm)` - форма для ***авторизации (login)*** ранее зарегистрированного пользователя системы
...
|---|---------------------|------------------------|-------------------------------------------------------|--------------------|--------------------|
| 1 |                     |                        |                                                       |                    |                    |
| 2 | **LoginPageView**   | `LoginView`            | Вход ранее зарегистрированного пользователя в систему | `AppUser`          | `AppUserLoginForm` |
|   |                     |                        |                                                       |                    |                    |
...
|---|-----------|------------|----------------------------------------------------------------------|
| 1 |           |            |                                                                      |
| 2 | `login/`  | `POST`     | Авторизация (Login) ранее зарегистрированного пользователя в системе |
| 3 | `logout/` | `POST`     | Выход (Logout)                                                       |
|   |           |            |                                                                      |
