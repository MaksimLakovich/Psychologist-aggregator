...
class SelectedSlotsMatcher(AbsTimeMatcher):
    """Основной пользовательский matcher - оставляет только тех специалистов, свободные слоты у которых
    совпадают с выбранными пользователем доменными слотами. Matcher обрабатывает одного специалиста за вызов.
    Используется для:
