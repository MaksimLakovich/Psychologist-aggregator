 ...
 *
 * Важное уточнение:
 * - это НЕ срок жизни серверного рандомного алгоритма и НЕ срок жизни данных в БД;
 * - это срок жизни только клиентской записи в sessionStorage (браузерная "память" для возврата).
 *
 * По сути это защитный барьер "на будущее":
 * - в основном сценарии мы и так возвращаемся через history.back();
 * - TTL нужен как аккуратная страховка для fallback-ветки, когда history.back() не подходит.
 * Стоимость такого механизма почти нулевая, а предсказуемость UX выше.
 */
