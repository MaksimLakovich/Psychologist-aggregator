...
// Шаг 2: Автосохранение выбора возраста психолога.
...
export function initAutosavePreferredAge({
    containerSelector,
    ...
    if (!containerSelector || !hiddenInputsSelector || !saveUrl) {
        console.error("initAutosavePreferredAge: missing required parameters");
        return;
    }

    const container = document.querySelector(containerSelector);
    if (!container) {
        console.warn("initAutosavePreferredAge: container not found:", containerSelector);
        return;
    }
    ...
    function doSave(values) {
        ...
        values.forEach(v => params.append("preferred_ps_age", v));
        ...
        return fetch(saveUrl, {
            ...
            .then((data) => {
                console.log("Автосохранение preferred_ps_age:", data);
            })
            .catch((err) => {
                console.error("Ошибка автосохранения preferred_ps_age:", err);
            });
    }
    ...
    container.addEventListener("click", (e) => {
        const btn = e.target.closest(".ps-age-btn");
        if (!btn) return;
    ...
    console.log("initAutosavePreferredAge: initialized");
}
