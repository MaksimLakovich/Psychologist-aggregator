"""Сервисный модуль для guest-flow подбора психолога.

Здесь в session хранится и потом восстанавливается временное состояние пользователя:
    - ответы на шаги подбора;
    - выбранный слот;
    - данные для возврата к бронированию после регистрации и подтверждения email.

Изначально модуль создавался для гостя.
Теперь часть его helpers также полезна и для psychologist:
например, когда специалист открывает карточку коллеги и система должна корректно показать расписание,
используя уже знакомую session-логику и безопасные значения по умолчанию.
"""
...
def get_guest_matching_state(session) -> dict[str, Any]:
    ...
    ВАЖНО! Это нужно не только для guest-flow.
    Теперь этот helper используется и в сценариях, где psychologist смотрит расписание коллег:
    backend может безопасно взять timezone и другие базовые значения из session, не ломая уже существующий flow.
    """
...
class AnonymousClientProfile:
    ...
    Благодаря этому backend может использовать один и тот же понятный объект:
        - в обычном guest-flow клиента;
        - и в сценариях, когда нужно показать "Расписание специалистов" для авторизованного пользователя под psychologist.
    """
...
def build_guest_profile(session) -> AnonymousClientProfile:
    ...
    ВАЖНО! Изначально функция нужна была для шагов подбора, когда пользователь еще не зарегистрирован.
    Теперь ее используют чуть шире:
        - по-прежнему в guest-flow клиента;
        - и в сценариях, где psychologist смотрит расписание коллег.

    Бизнес-смысл такой:
    система может взять из session понятный временный профиль с timezone и предпочтениями
    и корректно показать расписание, даже если в БД нет реального ClientProfile для этого сценария.
    """
