...
class ClientAddPaymentCardPageView(SpecialistMatchingLayoutMixin, LoginRequiredMixin, FormView):
    ...
    def get_initial(self):
        ...
        initial.update(
            {
                "specialist_profile_id": self.request.GET.get("specialist_profile_id", ""),
                "slot_start_iso": self.request.GET.get("slot_start_iso", ""),
    ...
    def form_valid(self, form):
        """Создает терапевтическую сессию после подтверждения на странице payment-card.
        ...
        Бизнес-смысл текущего этапа:
            ...
            - после submit сразу создается терапевтическая сессия и клиент получает подтверждение.
        """
